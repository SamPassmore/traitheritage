...
distance_trait_heritage = function(tree, generation_time, distance_matrix, cut_off){
  ...
  dm = data.table(row = gl(n, 1L, labels = nms[[1L]])[sequence(s, 1L)],
                  col = rep.int(gl(n, 1L, labels = nms[[2L]]), s),
                  value = distance_matrix[sequence(s, seq.int(1L, length(distance_matrix), n))])

  dm$trait = dm$value < cut_off

  # xx = dm[clades, on = .(row = taxa), allow.cartesian = TRUE]

  xx = clades[dm, on = .(taxa = row), allow.cartesian = TRUE]

  clades = dplyr::full_join(clades, data.frame(taxa = names(trait), trait = trait), by = "taxa")

  xx[,.clade_probabilities(trait), by = c("generation", "clade")]

  dm[,.clade_probabilities(trait), by = c("generation", "clade")]
